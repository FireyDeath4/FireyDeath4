...
//var cookies={person:{username:getCookie("username"),tasks:JSON.stringify(getCookie("tasks"))}}
//$("#cookies").html(JSON.stringify(cookies));
...
sector=['<path stroke="none" fill-rule="evenodd" d="','" fill="hsla(',')"></path>'],
text=['<text x="10" y="13" font-size="8" font-family="monospace" font-weight="bold" text-anchor="middle" text-baseline="middle" stroke="black" stroke-width="0.25" fill="hsl(',')">','</text>'],
...
function tickRender()
  ...
  dialsF.push(dial[0]+(Math.sin(newDate.getMilliseconds()/500*Math.PI)*(dialsF.length==0?7:9)+10)+
  dial[1]+(-Math.cos(newDate.getMilliseconds()/500*Math.PI)*(dialsF.length==0?7:9)+10)+
  ...
  for(run=0;run<times.length;run++){clock+='<svg height="20" width="20" viewBox="0 0 20 20" class="'+
    ...
    var temp={//inner sector
      //outer sector
      //dials
      //time value
      time:[drawAngle(0,dateProgs.time*360,10,10,10),
